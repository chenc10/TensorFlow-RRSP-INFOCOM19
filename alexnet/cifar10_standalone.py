...
#      network_fn = nets_factory.get_network_fn('inception-v3',num_classes=10) 
...
#  network_fn = nets_factory.get_network_fn('inception_v3',num_classes=1000) 
#  network_fn = nets_factory.get_network_fn('vgg_16',num_classes=10) 
#  network_fn = nets_factory.get_network_fn('resnet_v2_50',num_classes=10) 
...
#  logits = vgg16(inputs)
  ...
  boundaries = [int(_BATCHES_PER_EPOCH * epoch) for epoch in [100, 150, 200]]
  #values = [_INITIAL_LEARNING_RATE * decay for decay in [1, 0.1, 0.01, 0.001]]
  values = [_INITIAL_LEARNING_RATE * decay for decay in [0.1, 0.01, 0.001]]
  learning_rate = tf.train.piecewise_constant(
      tf.cast(global_step, tf.int32), boundaries, values)
...
#     tf.summary.scalar('train_accuracy', accuracy[1])
  ...
  for steps in range(1000):
    ...
    print(str(time.time()-time0)+' hey-step '+str(steps) + '; loss: ' + str(loss_value))
    if steps== 6:
